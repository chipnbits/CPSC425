...
'''
When you use np.asarray on an image object (like one from PIL, the Python Imaging Library), 
it converts the image to a NumPy array. However, this conversion does not create a new, 
independent array. Instead, the resulting array (im2_array in your case) is essentially 
a different view of the same underlying data. This means that modifications to the 
im2_array would directly affect the original image object (im2), because they both 
reference the same data in memory.
'''
