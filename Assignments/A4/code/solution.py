...
def FindBestMatches(descriptors1, descriptors2, threshold):
    ...
    # The first task is to compute the distance matrix when it comes to nearest neighbor
    # Since the descriptors have all been normalized to unit length, the distance can be
    # computed using the angle between them or the arc cosine of the dot product.
    
    angle_matrix = np.zeros((descriptors1.shape[0], descriptors2.shape[0]))
    nearest_neighbors = np.zeros((descriptors1.shape[0], 2), dtype=int)
    nearest_angles = np.zeros((descriptors1.shape[0], 2))
    
    for i in range(descriptors1.shape[0]):
        # Track the two nearest neighbors to i 
        for j in range(descriptors2.shape[0]):
            angle_matrix[i][j] = math.acos(np.dot(descriptors1[i], descriptors2[j]))
        
        # After computing all angles for descriptor i, find the indices of the two nearest neighbors
        sorted_indices = np.argsort(angle_matrix[i])[:2]
        nearest_neighbors[i] = sorted_indices
        nearest_angles[i] = angle_matrix[i][sorted_indices]
        
    # Now that we have the nearest neighbors, we can compute the ratio test
    # A low ratio indicates that the second neighbor is much further than the first and 
    # therefore the first is a more unique match
    ratio = nearest_angles[:, 0] / nearest_angles[:, 1]
    ratio_test = ratio < threshold
