...
def ComputeSSD(TODOPatch, TODOMask, textureIm, patchL):
	...
	for r in range(ssd_rows):
		for c in range(ssd_cols):
			...
			# Take the difference between the texture image and the TODOPatch
			diff = textureIm[r:r+2*patchL+1,c:c+2*patchL+1,:] - TODOPatch*1.0
			# Square the difference
			diff = diff ** 2
            # Apply the mask to all color channels.
            # The mask needs to be reshaped for broadcasting to work correctly.
			masked_diff = diff * (TODOMask == 0)[:,:,np.newaxis]
...
def CopyPatch(imHole,TODOMask,textureIm,iPatchCenter,jPatchCenter,iMatchCenter,jMatchCenter,patchL):
	...
	for i in range(patchSize):
		for j in range(patchSize):
			...
			if TODOMask[i][j] == 1:
				imHole[i + iPatchCenter - patchL][j + jPatchCenter - patchL] \
    			= textureIm[i + iMatchCenter - patchL][j + jMatchCenter - patchL]
			else:
				# If not part of the TODOmask then dont copy
				pass
...
im = Image.open('ocean.jpg').convert('RGB')
...
if os.path.isfile('fill_region.pkl') and os.path.isfile('texture_region.pkl'):
	fill_region_file = open('fill_region_ocean.pkl', 'rb')
	fillRegion = pickle.load( fill_region_file )
	fill_region_file.close()

	texture_region_file = open('texture_region_ocean.pkl', 'rb')
...
# nFill is the length of the list of fill indices
while (nFill > 0):
	...
	# Onion fill style algorithm: process all boundary pixels before
	# starting to fill in the hole. All pixels in the TODORegion get 
    # filled before doing apply edge again. 

	while(nTODO > 0):

		# Pick a random pixel from the TODORegion
  		# edge_pixels is 2xnTODO array with row and column coordinates 
